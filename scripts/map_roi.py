...
def robust_task_execution(func):
    ...
    def wrapper(
        ...
        pair_label = f"{task.image_id_to}_vs_{task.image_id_from}"
...
class PalomReaderFactory:
    ...
    def create_reader(
        ...
        # FIXME: Custom props that do not exist in DaPyramidChannelReader
...
def _generate_qc_plot(task: AlignmentTask) -> Optional[str]:
    ...
    try:
        ...
        fig.suptitle(
            f"Coarse Alignment QC: {task.image_id_to} (Ch {task.channel_to}) vs {task.image_id_from} (Ch {task.channel_from})",
            fontsize=10,
        )
        ax.set_title(
...
def run_single_alignment_task(
    ...
    handler1 = ImageHandler(conn, task.image_id_to)
    handler2 = ImageHandler(conn, task.image_id_from)
    if not handler1.is_valid():
        raise ValueError(
            f"Image handler failed to initialize for Image ID: {task.image_id_to}"
        )
    if not handler2.is_valid():
        raise ValueError(
            f"Image handler failed to initialize for Image ID: {task.image_id_from}"
        )

    reader1 = PalomReaderFactory.create_reader(
        handler1, task.channel_to, task.max_pixel_size
    )
    reader2 = PalomReaderFactory.create_reader(
        handler2, task.channel_from, task.max_pixel_size
    )
    ...
    if task.map_rois and not task.dry_run:
        log.info(
            f"[{pair_label}] Mapping ROIs from {task.image_id_from} to {task.image_id_to}..."
        )
        try:
            transformed_shapes = _fetch_and_transform_rois(
                conn, task.image_id_from, affine_matrix
            )
            rois_mapped = _post_rois(conn, task.image_id_to, transformed_shapes)
...
def main():
    ...
    mode_group.add_argument(
        ...
        "Required columns: image_id_to, image_id_from, channel_to, channel_from.",
    )
    parser.add_argument(
        "--image-id-from",
        type=int,
        metavar="ID",
        help="Omero Image ID of the image which the ROIs are from",
    )
    mode_group.add_argument(
        "--image-id-to",
        type=int,
        metavar="ID",
        help="Omero Image ID of the image which the ROIs are being mapped and transferred to. Required if --image-id-from is provided.",
    )
    ...
    parser.add_argument(
        "--map-rois",
        action="store_true",
        help="Attempt to map ROIs from image_id_from to image_id_to. Ignored if --dry-run is set.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        ...
        if args.batch_csv:
            ...
            log.info(
                f"Starting Single Pair Mode for: {args.image_id_to} vs {args.image_id_from}"
            )
            task = AlignmentTask(
